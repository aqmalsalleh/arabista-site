...
**Version:** 3.3 | **Aligned to codebase:** May 10, 2026  
...
**v3.3 (10 May):** **Tri-platform analytics restoration (`arabista-core.js`, v4 PDP).** `arabista-core.js` introduces `ttqSafe(event, name, payload)` alongside `gtagSafe` / `fbqSafe` and centralises the `AddToCart` fan-out **inside `Cart.addItem`** so every cart mutation fires GA4 `add_to_cart`, Meta `AddToCart`, and TikTok `AddToCart` exactly once with proper payload mapping (`items: [{item_id,item_name,price,quantity}]` for GA4; `content_ids` for Meta; `content_type:'product', content_id, description` for TikTok). The `doCheckout` flow now fires `begin_checkout` (GA4) and `InitiateCheckout` (Meta + TikTok) **before** the `reserve_stock` fetch, passing `value`, `currency:'MYR'`, and the items array, so funnel signal is captured even on OOS rejects. `product-z01-v4.html` `<head>` now carries the standard base scripts: Meta Pixel **`1400612660505177`**, GA4 **`G-13C2S23N6T`**, TikTok Pixel **`CP5R3JBC77U0I1KMM47G`** (the legacy `G-Q6KYKNKZW6` GA4 tag is left in place — see “Pixel hygiene” below). The legacy v3.1 inline `gtagSafe` / `fbqSafe` calls inside `addCurrentToCart` were removed to prevent double-firing.
