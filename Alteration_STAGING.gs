        ...
        if (response.code === 200) {
          ...
          if (newStatus !== inboundStatus) {
            ...
            if (newStatus === "ON_GOING" && String(inboundStatus).trim() === "ASSIGNING_DRIVER") {
              ...
              const custMsg = `*[LALAMOVE SECURED]*\nOrder: ${altId}\n\nYour inbound Lalamove rider is on the way to pick up your Abaya.\n🔗 Track Rider: ${trackingUrl}\n🔗 Order Dashboard: ${trackerUrl}`;
              sendWhatsAppText(custPhoneNum, custMsg);
        ...
        if (response.code === 200) {
          ...
          if (newStatus !== outboundStatus) {
            ...
            if (newStatus === "ON_GOING" && String(outboundStatus).trim() === "ASSIGNING_DRIVER") {
              ...
              const custMsg = `*[LALAMOVE DISPATCHED]*\nOrder: ${altId}\n\nPayment successful! Your return Lalamove rider is on the way to pick up your Abaya from the hub and deliver it to you.\n\nOnce you have received your items, please click the button below to close the order.\n\n🔗 Track Rider: ${trackingUrl}`;
            ...
            if (newStatus === "COMPLETED") {
              ...
              const custMsg = `*[ORDER COMPLETED]*\nOrder: ${altId}\n\nYour Abaya has been successfully delivered by Lalamove, and this order is now officially closed. Thank you for choosing Arabista!\n🔗 ${trackerUrl}`;
              sendWhatsAppText(custPhoneNum, custMsg);
...
function processWhatsAppIntent(senderPhone, aiAnalysis, isHub = false) {
  ...
  if (intent === "CUSTOMER_ACTIVATION" || intent === "CUSTOMER_FOLLOWUP") {
    ...
    const trackerUrl = `https://arabistaofficial.com/alteration-tracker-staging.html?id=${orderId}`;
    const custMsg = `*[ITEM RECEIVED]*\nOrder: ${orderId}\n\nYour Abaya has safely arrived at the hub! Our tailor is now beginning the alteration process. We will notify you the moment it is ready.\n🔗 ${trackerUrl}`;
    sendWhatsAppText(custPhone, custMsg);
    touchDbUpdate();
  }
    ...
    const custMsg = `*[ORDER COMPLETED]*\nOrder: ${orderId}\n\nYour Abaya has been successfully handed over, and this order is now officially closed. Thank you for choosing Arabista!\n🔗 ${trackerUrl}`;
    sendWhatsAppText(custPhone, custMsg);
    touchDbUpdate();
  }
}
  ...
  try {
    ...
    if (changes.messages && changes.messages.length > 0) {
      ...
      if (msg.type === "text") {
        ...
        let aiAnalysis = parseMessageWithGemini(incomingText);
...
function processInteractivePayload(phone, action, orderId) {
  ...
  if (action === "HUB_ACCEPT" || action === "HUB_REJECT") {
    ...
    if (action === "HUB_ACCEPT") {
      ...
      const custMsg = `*[ACTION REQUIRED]*\nOrder: ${orderId}\n\nGreat news! The hub has accepted your order.\n\nPlease click the link below to choose your inbound delivery method (Lalamove or Hub Drop-off) and send your Abaya to the tailor.\n🔗 ${trackerUrl}`;
      sendWhatsAppText(custPhone, custMsg);
      touchDbUpdate();
      return { success: true };
    }
  ...
  // ----------------------------------------------------------------------
  // STATE 2: HUB_RECEIVED (Abayas Arrived)
  // ----------------------------------------------------------------------
    ...
    // TP3: Abayas Received (Customer Message)
    const custMsg = `*[ITEM RECEIVED]*\nOrder: ${orderId}\n\nYour Abaya has safely arrived at the hub! Our tailor is now beginning the alteration process. We will notify you the moment it is ready.\n🔗 ${trackerUrl}`;
    sendWhatsAppText(custPhone, custMsg);
    touchDbUpdate();
    return { success: true };
  ...
  // ----------------------------------------------------------------------
    ...
    // TP6: Final Closure (Customer Message)
    const custMsg = `*[ORDER COMPLETED]*\nOrder: ${orderId}\n\nThank you for confirming! Your order is now officially closed. We hope you love your newly altered Abaya!\n🔗 ${trackerUrl}`;
